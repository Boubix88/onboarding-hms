...
function CreateLoan() {
  ...
  function onSubmitHandler(data: { bien: string; autreBien?: string; montant: number; taux: number; duree: number }) {
    console.log(data);
    reset();
    ...
    // On crée le prêt
    LoanAPI.addLoan(loan).then((loanResponse) => {
      setLoan(loan);
      setShowLoanSimulation(true);

      setNotification({ message: "Sauvegarde de la simulation de prêt", type: 'success' });
      setShowNotification(true);
    }).catch((error) => {
      setNotification({ message: 'Erreur lors de la sauvegarde de la simulation', type: 'error' });
      setShowNotification(true);
    });
  }
